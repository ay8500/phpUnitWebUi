...
function getTestFiles(project) {
    $.ajax({
        url:'ajaxGetTestFiles.php?project='+project,
        type:'GET',
        success:function(data){
            testFiles=data;
            while (filesData.getNumberOfRows()>0)
            ...
            filesOption.title ='PhpUnit projects:'+projects+' files:'+filesData.getNumberOfRows()+' tests:'+tests+" asserts:"+asserts;
            filesChart.draw(filesData, filesOption );
        <?php if($pu->getGetParam("action")=="autorun") {?>
            runAlltests();
         <?php }?>
